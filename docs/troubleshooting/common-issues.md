...
# Check events
kubectl get events -n dclaw-rag --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-rag deployment/dclaw-rag-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-rag-db-credentials -n dclaw-rag
   ```
